...
class CaseService:
    ...
    async def get(self):
        try:
            async with self.session.get(f"{self.base_url}/cases/") as response:
    ...
    async def get_by_id(self, id):
        try:
            async with self.session.get(f"{self.base_url}/cases/{id}") as response:
                if response.status == 200:
                    return await response.json()
                elif response.status == 404:
                    return None
    ...
    async def create(self, data: CreateCase):
        try:
            async with self.session.post(
                f"{self.base_url}/cases/{id}", data=data.model_dump()
            ) as response:
                if response.status == 200:
                    return await response.json()
                elif response.status == 404:
    ...
    async def delete(self, id):
        try:
            async with self.session.delete(f"{self.base_url}/cases/{id}") as response:
                if response.status == 200:
                    return await response.json()
                elif response.status == 404:
                    return None
