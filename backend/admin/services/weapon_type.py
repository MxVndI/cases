...
class WeaponTypeService:
    ...
    async def get_all(self):
        try:
            async with self.session.get(f"{self.base_url}/weapon-types/") as response:
    ...
    async def get_by_id(self, id: str):
        try:
            async with self.session.get(f"{self.base_url}/weapon-types/{id}") as response:
                if response.status == 200:
    ...
    async def create(self, name: str):
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/weapon-types/", json=payload
            ) as response:
                if response.status == 200:
                    return self._normalize_entity(await response.json())
                elif response.status == 404:
    ...
    async def update(self, id: str, name: str | None = None):
        try:
            ...
            async with self.session.patch(
                f"{self.base_url}/weapon-types/{id}", json=payload
            ) as response:
                if response.status == 200:
                    return self._normalize_entity(await response.json())
                elif response.status == 404:
    ...
    async def delete(self, id: str):
        try:
            async with self.session.delete(
                f"{self.base_url}/weapon-types/{id}",
