...
class WeaponService:
    ...
    async def get_all(self):
        try:
            async with self.session.get(f"{self.base_url}/weapons/") as response:
    ...
    async def get_by_id(self, id: str):
        try:
            async with self.session.get(f"{self.base_url}/weapons/{id}") as response:
                if response.status == 200:
    ...
    async def create(self, name: str, type: str):
        try:
            payload = {"name": name, "type": type, "token": self.settings.token}
            async with self.session.post(
                f"{self.base_url}/weapons/", json=payload
            ) as response:
                if response.status == 200:
                    return self._normalize_entity(await response.json())
                elif response.status == 404:
    ...
    async def update(self, id: str, name: str | None = None, type: str | None = None):
        try:
            ...
            async with self.session.patch(
                f"{self.base_url}/weapons/{id}", json=payload
            ) as response:
                if response.status == 200:
                    return self._normalize_entity(await response.json())
                elif response.status == 404:
